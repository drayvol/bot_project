"""
build_equation_index.py — этап 3: векторная база уравнений Решу ЕГЭ в Qdrant.

Источник: sdamgia_task13.jsonl (parse_sdamgia.py) — задачи №13 с решениями.
Пайплайн:
  1. Уравнение (LaTeX) → TF-IDF (символьные n-граммы) → TruncatedSVD → L2-норма.
  2. Qdrant (embedded, ./qdrant_db), payload: уравнение, решение, ответ,
     категория, url, аналоги.
  3. Векторизатор → equation_vectorizer.joblib (им же кодируются запросы).

Запуск:  python build_equation_index.py
"""
...
SOURCE          = 'data/sdamgia_task13.jsonl'
...
def main():
    ...
    print(f'sdamgia: {len(recs)}, golden set (gt=1): {len(golden)}')
    recs = recs + golden

    # дедупликация по нормализованному уравнению (sdamgia в приоритете)
    seen, data = set(), []
    for r in recs:
